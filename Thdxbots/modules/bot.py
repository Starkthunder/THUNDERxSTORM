...
@X8.on(events.NewMessage(incoming=True, pattern=r"\%sping(?: |$)(.*)" % hl))ʀᴇsᴛᴀʀᴛɪɴɢ
@X9.on(events.NewMessage(incoming=True, pattern=r"\%sping(?: |$)(.*)" % hl))
@X10.on(events.NewMessage(incoming=True, pattern=r"\%sping(?: |$)(.*)" % hl))
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» __⚡ᴛʜᴜɴᴅᴇʀ⚡__")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await thunder.edit(f"__🌪ꜱᴛᴏʀᴍ🫧__\n» `{mp} ʙᴏʟᴛ`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`ʀᴇ-ꜱᴛᴀʀᴛɪɴɢ ᴛʜᴇ ꜱᴛᴏʀᴍᴄʟᴏᴜᴅ⛈️...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __ᴀᴅᴅɪɴɢ ᴜꜱᴇʀ ɪɴ ᴛʜᴜɴᴅᴇʀ ᴄʟᴏᴜᴅ ✨...__")
        target = ""
